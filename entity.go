...
type StarPagoDepositReq struct {
	...
	Extra      StarPagoDepositReqExtra `json:"extra" mapstructure:"extra"`
	// 以下sdk帮搞
	// AppID string `json:"appId"` // 应用号, appID
	// NotifyURL  string                  `json:"notifyUrl"`           // 异步通知地址, 异步通知地址
	// ReturnURL  *string                 `json:"returnUrl,omitempty"` // 同步通知地址, 同步通知地址
	// PayMethod  string                  `json:"payMethod"`        // fixed: PK_WALLET
	// Sign string `json:"sign"` // 签名
}

type StarPagoDepositReqExtra struct {
	PayType  string  `json:"payType" mapstructure:"payType"`             // PK_WALLET_JAZZCASH / PK_WALLET_EASYPAISA 其中一个
...
type StarPagoWithdrawReq struct {
	...
	Attach     string                   `json:"attach" mapstructure:"attach"`         // 附加信息, 商户附加信息，原样返回
	Currency   string                   `json:"currency" mapstructure:"currency"`     // 金额币种, PKR
	Extra      StarPagoWithdrawReqExtra `json:"extra" mapstructure:"extra"`           // 扩展信息
	MerOrderNo string                   `json:"merOrderNo" mapstructure:"merOrderNo"` // 商户订单号, 商户订单号
	//以下sdk设置
	//AppID string `json:"appId"` // 应用号, appID
	//NotifyURL string `json:"notifyUrl"` // 异步通知地址, 异步通知地址
...
// 扩展信息
type StarPagoWithdrawReqExtra struct {
	// 持卡人姓名, 收款人姓名(尽量真实)
